...
export class DoctorsManagementComponent implements OnInit {
  ...
  errorMessage = '';
  successMessage = '';
  createdDoctorCredentials: { email: string; tempPassword: string; verificationCode: string; emailSent: boolean } | null = null;

  // Options pour les champs

  specialties = [
    { code: 'PNEUMOLOGIE', label: 'Pneumologie' },
    { code: 'ONCOLOGIE', label: 'Oncologie' },
    { code: 'NEUROLOGIE', label: 'Neurologie' },
    { code: 'CARDIOLOGIE', label: 'Cardiologie' },
    { code: 'DERMATOLOGIE', label: 'Dermatologie' },
    { code: 'OPHTALMOLOGIE', label: 'Ophtalmologie' },
    { code: 'GYNECOLOGIE', label: 'Gynécologie' },
    { code: 'GASTROENTEROLOGIE', label: 'Gastroentérologie' },
    { code: 'RADIOLOGIE', label: 'Radiologie' },
    { code: 'MEDECINE_GENERALE', label: 'Médecine générale' }
  ];
  ...
  createForm(): FormGroup {
    return this.fb.group({
      ...
      // Informations professionnelles (specialization dérivée du specialtyCode)
      specialization: [''],
  ...
  // Mise à jour automatique de specialization quand specialtyCode change
  onSpecialtyCodeChange(code: string): void {
    const found = this.specialties.find(s => s.code === code);
    if (found) {
      this.doctorForm.get('specialization')?.setValue(found.label);
    }
  }
  ...
  onSubmit(): void {
    ...
    if (this.isEditing && this.editingId) {
      ...
      this.doctorService.createDoctor(doctorData).subscribe({
        next: (response) => {
          this.successMessage = response.message || 'Médecin créé avec succès !';
          if (response.tempPassword && response.verificationCode) {
            this.createdDoctorCredentials = {
