...
public class DataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        try {
            // Créer automatiquement l'admin Said Nour
            CreateAdminRequest saidNourRequest = CreateAdminRequest.builder()
                    .name("said nour")
                    .email("noour.said1803@gmail.com")
                    .password("Admin123!")
                    .build();

            adminService.createAdmin(saidNourRequest);
            log.info("✅ Admin 'said nour' créé automatiquement !");
            log.info("📧 Email: noour.said1803@gmail.com");
            log.info("🔑 Mot de passe temporaire: Admin123!");
            log.info("⚠️  IMPORTANT: Un code de vérification a été envoyé à l'email.");
            log.info("📝 Vérifiez l'email et utilisez le code pour activer le compte.");

        } catch (RuntimeException e) {
            if (e.getMessage().contains("existe déjà")) {
                log.info("ℹ️  Admin 'said nour' existe déjà, pas de création nécessaire.");
            } else {
